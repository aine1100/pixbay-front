...
export function RecentBookings() {
    ...
    const bookings = (rawBookings || []).map((b: any) => ({
        ...
        creator: `${b.creator?.user?.firstName || ''} ${b.creator?.user?.lastName || ''}`,
        ...
        avatar: b.creator?.user?.profilePicture || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=100"
    }));
    ...
    return (
        <div className="bg-white p-6 rounded-[32px] border border-slate-100 overflow-hidden flex flex-col h-full">
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <p className="text-[11px] text-primary font-semibold truncate max-w-[100px]">{booking.creator}</p>
                                ...
                                <div className="text-right">
                                    <span className={cn(
                                        ...
                                        ["Approved", "Confirmed", "Completed"].includes(booking.status) ? "bg-red-50 text-primary" : "bg-slate-100 text-slate-600"
                                    )}>
                                        {booking.status}
                                    </span>
            ...
            <Link href="/client/bookings" className="w-full py-4 mt-6 text-xs font-semibold text-slate-700 hover:text-primary transition-colors flex items-center justify-center gap-2 border-t border-slate-100 uppercase tracking-widest">
                View All Bookings
